...
/* Circle Variables */
var visibleState1 = hihatCircle.map(({ visible }) => visible);
var visibleState2 = snareCircle.map(({ visible }) => visible);
var visibleState3 = kickCircle.map(({ visible }) => visible);
var hihatStates = hihatCircle.map(({ checked }) => checked);
var snareStates = snareCircle.map(({ checked }) => checked);
var kickStates = kickCircle.map(({ checked }) => checked);

/* Main Variables */
var navCheck = false;
var isPlaying = false;
var isRecording = false;
var selectedPresetModul;
var hihatSliderVal = 0;
var snareSliderVal = 0;
var kickSliderVal = 0;

/* Settings Variables */
var menuCheck;
var useBPM = 100;
var bpmInterval = 240000 / useBPM;
var pulseInterval = bpmInterval / 8;
var beatCycle;
var useTimeSig = 'Free';
...
var unlockedSamples = soundList.map(({ disabled }) => disabled);
var selectedSoundNameSaved = 'Acoustic';
var selectedSoundIndexSaved = 0;
var libraryLoaded = false;
var fileName = 'Ritmo_MIDI';
...
async function loadSounLib() {
  if (libraryLoaded == false) {
    /*
    await hihatMP3.loadAsync(
      Sounds[soundList[selectedSoundIndexSaved].hihatSound]
    );
    await snareMP3.loadAsync(
      Sounds[soundList[selectedSoundIndexSaved].snareSound]
    );
    await kickMP3.loadAsync(
      Sounds[soundList[selectedSoundIndexSaved].kickSound]
    );

    await hihatMP3.setVolumeAsync(0.8);
    await snareMP3.setVolumeAsync(0.8);
    await kickMP3.setVolumeAsync(0.8);
    */

    var hihatPath = soundList[selectedSoundIndexSaved].hihatSound;
    var snarePath = soundList[selectedSoundIndexSaved].snareSound;
    var kickPath = soundList[selectedSoundIndexSaved].kickSound;

    if (Platform.OS === 'ios') {
      hihatPath = 'samples/' + soundList[selectedSoundIndexSaved].hihatSound;
      snarePath = 'samples/' + soundList[selectedSoundIndexSaved].snareSound;
      kickPath = 'samples/' + soundList[selectedSoundIndexSaved].kickSound;
    }

    hihatMP3 = new RNSound(hihatPath, RNSound.MAIN_BUNDLE);
    snareMP3 = new RNSound(snarePath, RNSound.MAIN_BUNDLE);
    kickMP3 = new RNSound(kickPath, RNSound.MAIN_BUNDLE);
...
async function unloadSoundLib() {
  // await hihatMP3.unloadAsync();
  // await snareMP3.unloadAsync();
  // await kickMP3.unloadAsync();
...
function calcSoundDelay() {
  ...
  var bpmAdjust = (60 / useBPM) * 4;
  var delayDegree = (1000 * bpmAdjust) / 360;

  for (var i in hihatCircle) {
    var delaySound,
  ...
  for (var i in hihatCircle) {
    if (hihatCircle[i].checked == true) {
      hihatCircle[i].soundDelayKey = setTimeout(async function () {
        // await hihatMP3.setPositionAsync(0);
        // await hihatMP3.playAsync(0);
        hihatMP3.setCurrentTime(0);
        hihatMP3.play();
  ...
  for (var i in snareCircle) {
    if (snareCircle[i].checked == true) {
      snareCircle[i].soundDelayKey = setTimeout(async function () {
        // await snareMP3.setPositionAsync(0);
        // await snareMP3.playAsync(0);
        snareMP3.setCurrentTime(0);
        snareMP3.play();
  ...
  for (var i in kickCircle) {
    if (kickCircle[i].checked == true) {
      kickCircle[i].soundDelayKey = setTimeout(async function () {
        // await kickMP3.setPositionAsync(0);
        // await kickMP3.playAsync(0);
        kickMP3.setCurrentTime(0);
        kickMP3.play();
...
async function askForReview() {
  ...
  var timeDiff = reviewWait - currentDate;
  ...
  if (timeDiff <= 0) {
    ...
    if (
      ...
      (await InAppReview.isAvailableAsync()) &&
      (await InAppReview.hasAction())
    ) {
  ...
  return (
    <View style={styles.guideWrapper}>
      ...
      <ScrollView style={styles.guideScroll}>
        ...
        <Text style={styles.guideTxt}>
          <Text style={{ fontFamily: 'PrimaryFont' }}>Save</Text>
          {'\n'}
        ...
        <Text style={styles.guideTxt}>
          ...
          can now be loaded.{'\n'}
          {'\n'}
          <Text style={{ fontFamily: 'PrimaryFont' }}>Load</Text>
          {'\n'}
          To load the preset just tap it.{'\n'}
          {'\n'}
          <Text style={{ fontFamily: 'PrimaryFont' }}>Delete</Text>
          {'\n'}
          To clear/delete the preset{' '}
          <Text style={{ fontFamily: 'PrimaryFont' }}>long-press</Text> it and a
        ...
        <Text style={styles.guideTxt}>
          ...
          <Text style={{ fontFamily: 'PrimaryFont' }}>Ritmo_MIDI</Text>" will be
          used){'\n'}
          {'\n'}
        ...
        <View style={styles.guideImgCont}>
          <Image
            ...
            resizeMode={'contain'}
            source={require('../assets/img/midiFile.png')}
          />
        </View>
        <Text style={styles.guideTxt}>
        ...
        <View style={styles.guideImgCont2}>
          <Image
            style={styles.guideImg}
            resizeMode={'contain'}
            source={require('../assets/img/midiFile_Logic.png')}
          />
        </View>
        <Text style={styles.guideTxt}>
...
export const MenuScreen = forwardRef((props, ref) => {
  ...
  const bpmCheck = value => {
    var valueNum = Math.trunc(Number(value));
    if (valueNum < 1) {
      setBpmNumber('1');
      useBPM = 1;
        ...
        <View style={styles.menuWrapper}>
          ...
          <View style={styles.selectWrapper}>
            ...
            <TouchableOpacity
              ...
              <Arrow style={styles.selectArrow} />
            </TouchableOpacity>
          </View>
...
export const RhythmScreen = forwardRef((props, ref) => {
  ...
  var hihatRotation = hihatCircle.map(({ angle }) => angle);
  var snareRotation = snareCircle.map(({ angle }) => angle);
  var kickRotation = kickCircle.map(({ angle }) => angle);
  ...
  const rotateHihatCircle = value => {
    ...
    for (var i in hihatCircle) {
      var rotationUpdate = hihatCircle[i].initAngle + Number(value);
  ...
  const rotateSnareCircle = value => {
    ...
    for (var i in snareCircle) {
      var rotationUpdate = snareCircle[i].initAngle + Number(value);
  ...
  const rotateKickCircle = value => {
    ...
    kickSliderVal = value;
    for (var i in kickCircle) {
      var rotationUpdate = kickCircle[i].initAngle + Number(value);
  ...
  const startRec = async () => {
    ...
    var status = (await Audio.getPermissionsAsync()).granted;
...
function Test() {
  ...
  function slideNavEffect() {
    var slideYVal = -deviceWidth * 1.3;
    var slideXVal = deviceWidth;
  ...
  const exportMIDI = async value => {
    ...
    for (var i = 0; i < stepsInBar; i++) {
      var hihatIndex = hihatMIDI.indexOf(countAngle);
      var snareIndex = snareMIDI.indexOf(countAngle);
      var kickIndex = kickMIDI.indexOf(countAngle);
      ...
      var newCount = countAngle + sliderStep;
      countAngle = newCount;
    }
    var shiftLayout = notesLayout.slice();

    /* Calc note start point */
    var countStart = 0;
    var prevStart = 0;
    var startTicks = [];
    for (var i = 0; i < shiftLayout.length; i++) {
      if (shiftLayout[0] !== 'R') {
        var calcTicks = countStart * midiNoteMin + prevStart;
    ...
    /* Calc note duration */
    var prevNote = midiBarTicks;
    var notesTicks = [];
    for (var i = startTicks.length - 1; i >= 0; i--) {
      var calcNoteTick = prevNote - startTicks[i];
      if (calcNoteTick > midiNoteMax) {
        calcNoteTick = midiNoteMax;
      }
      var noteTick = 'T' + calcNoteTick;
      notesTicks.unshift(noteTick);
      prevNote = startTicks[i];
    }

    /* Create midi note layout array without rests (R) */
    var midiLayout = notesLayout.filter(i => i !== 'R');

    /* Write midi sequance */
    var track = new MidiWriter.Track();
    var notesMIDI = [];
    for (var i = 0; i < midiLayout.length; i++) {
      if (midiLayout[i] === 'KSH') {
        notesMIDI.push(
    ...
    /* Close midi modal & reset custom name */
    exitMidiModal();

    /* Write .mid file to app storage */
    const write = new MidiWriter.Writer(track);
    var fileUri = RNFS.documentDirectory + `${encodeURI(value)}.mid`;
